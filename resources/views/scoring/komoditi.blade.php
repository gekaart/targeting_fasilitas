...
@if ($scoring == 'yes')
    <form action="{{ url('/scoring/update/'.$npwp) }}" method="post">
        @method('put')
    @else
    <form action="{{ url('/scoring/store_komoditi/'.$npwp) }}" method="post">
    @endif
    @csrf
    {{ explode("/",request()->path())[0] }}
        <div class="container mt-5">
            ...
            <a href="{{ url(explode("/",request()->path())[0].'/tonaseCIF/'.$npwp) }}" class="btn btn-tranparant text-primary">Tonase dan CIF</a>
